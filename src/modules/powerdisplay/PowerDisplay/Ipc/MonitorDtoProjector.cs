...
{
    // ─── Canonical setting names (mirrors GetCommand.AllSettingNames) ───────────

    /// <summary>
    /// Canonical setting names accepted by the <c>--setting</c> filter. The same identifiers
    /// appear in <see cref="CliSettingValue.Setting"/> so JSON consumers can switch on them.
    /// </summary>
        ...
        string? settingFilter)
    {
        ...
        // No selector → emit every visible monitor (mirrors GetCommand.EmitAll)
        if (!number.HasValue && string.IsNullOrEmpty(id))
        {
            // An unknown --setting is monitor-independent; validate once up front.
            if (TryGetUnknownSettingError(settingFilter, out var settingErr))
            {
                return (null, new CliErrorResult { Command = "get", Error = settingErr! });
            }
        ...
        // Warning is surfaced to the caller via the returned result — the projector itself is
        // stateless (no ICliOutput). Callers should check the warning string on the resolution
        // tuple; here we proceed with the resolved monitor.
        _ = warning; // consumed by caller if needed; see overload below
        ...
        {
            return (null, new CliErrorResult { Command = "get", Monitor = mRef, Error = settingError });
        }
        ...
        string? settingFilter)
    {
        ...
        if (!number.HasValue && string.IsNullOrEmpty(id))
        {
            if (TryGetUnknownSettingError(settingFilter, out var settingErr))
            {
                return (null, new CliErrorResult { Command = "get", Error = settingErr! }, null);
            }
        ...
        if (resolveError is not null)
        {
            return (null, new CliErrorResult { Command = "get", Error = resolveError }, warning);
        }
        ...
        if (settingError is not null)
        {
            return (null, new CliErrorResult { Command = "get", Monitor = mRef, Error = settingError }, warning);
        }
    ...
    {
        error = null;
        if (settingFilter is null || Array.IndexOf(AllSettingNames, settingFilter.ToLowerInvariant()) >= 0)
        {
            return false;
        }
